...
- 109 hard-coded hex values (35 distinct), 140 `rgba()` values (66 distinct), 53 gradients and 7 gradient-text treatments sit alongside only 8 named tokens.
...
| `--success` | `#1E7A4D` | — |
| `--warning` | `#A76A00` | streaks and warnings |
| `--danger` | `#B4362C` | — |
...
4. Migrate 109 hex and 140 `rgba()` literals onto tokens; the contrast failures get fixed as part of that pass, not separately.
